...
void Sys_Error( const char *error, ... ) {
	...
	va_end( argptr );
	 ...
	 __android_log_print(ANDROID_LOG_ERROR,"JK2","%s", string);
...
void Sys_Warn( char *warning, ... ) {
	...
	((void)__android_log_print(ANDROID_LOG_INFO,"RTCW","%s", string));
...
void  Sys_Print( const char *msg ) {
	...
	((void)__android_log_print(ANDROID_LOG_INFO,"RTCW","%s", msg));
