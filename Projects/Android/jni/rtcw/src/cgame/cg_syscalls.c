...
#ifdef AUTOAIM
void        trap_SetUserCmdValue( int stateValue, int holdableValue, float sensitivityScale, int cld, int lastFire, int crossHairStatus ) {    //----(SA)	// NERVE - SMF - added cld
	syscall( CG_SETUSERCMDVALUE, stateValue, holdableValue, PASSFLOAT( sensitivityScale ), cld, lastFire, crossHairStatus );
	#else
